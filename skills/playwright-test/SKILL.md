...
See [references/github-actions.md](references/github-actions.md) for: required Linux CJK fonts, a basic (unsharded) workflow, shard execution and shard x browser matrix setups with report merging, retry strategy, choosing trace/screenshot/video capture, browser-conditional tests, and a flaky-detection workflow using the JSON reporter.
